...
# Matrix exponential
A_exp_list = []
for i in range(1, args.expm+1):
    A_exp = approx_expm(A, i)
    A_exp = preprocess_adj(A_exp, SYM_NORM, 'none')
    A_exp_list.append(A_exp)
...
for trial in range(args.ntrials):
    ...
    H_list = []
    G_list = []
    for i in range(len(A_exp_list)):
        H_list.append(X_in)
        # H_list.append(Dropout(0.5)(X_in))
        G_list.append(Input(shape=(None, None), batch_shape=(None, None), sparse=True))

    for i in range(args.nlayers-1):
        for j in range(len(A_exp_list)):
            H_list[j] = GraphConvolution(N_FILTERS, support, activation='relu', kernel_regularizer=l2(5e-4), bias_regularizer=l2(5e-4))([H_list[j], G_list[j]])
            H_list[j] = Dropout(0.5)(H_list[j])
    if args.expm > 1:
        H = Concatenate()(H_list)
    else:
        H = H_list[0]

    Y = GraphConvolution(n_classes, support, activation='softmax', kernel_regularizer=l2(5e-4), bias_regularizer=l2(5e-4))([H, G_list[0]])
    # Y = Dense(n_classes, activation='softmax', kernel_regularizer=l2(5e-4), bias_regularizer=l2(5e-4), name='classification')(H)

    input_list = [X_in] + G_list
    output_list = Y
    graph = [X] + A_exp_list
